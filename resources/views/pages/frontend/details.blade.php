    ...
    <section class="container mx-auto">
        <div class="flex flex-wrap my-4 md:my-12">
                ...
                <div class="slider">
                        ...
                        @foreach($products->galleries as $item)
                            <div class="px-2">
                                <div class="item {{ $loop->first ? 'selected' : '' }}" data-img="{{ Storage::url($item->url) }}">
                                    <img src="{{ Storage::url($item->url) }}" alt="front" class="object-cover w-full h-full rounded-lg"/>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    <div class="preview">
                        <div class="item rounded-lg h-full overflow-hidden">
                            <img
                                src="{{ $products->galleries()->exists() ? Storage::url($products->galleries->first()->url) : 'data:image/gif;base64,R0lGODlhAQABAIAAAAUEBAAAACwAAAAAAQABAAACAkQBADs=' }}"
                                alt="front"
            ...
            <div class="flex-1 px-4 md:p-6">
                <h2 class="text-5xl font-semibold">{{ $products->name }}</h2>
                <p class="text-xl">IDR {{ number_format($products->price) }}</p>

                <form action="{{ route('frontend.cart-add', $products->id) }}" method="post">
                    @csrf
                    <button
                        type="submit"
        ...
        <div class="container mx-auto">
            <div class="flex flex-start mb-4">
                <h3 class="text-2xl capitalize font-semibold">
                    Recommendation Laptop <br class="" />
                </h3>
            </div>
                    ...
                    <div class="px-3 flex-none" style="width: 320px">
                        <div class="rounded-xl p-4 pb-8 relative bg-white">
                            <div class="rounded-xl overflow-hidden card-shadow w-full h-36">
                                <img
                                    src="{{ $recomendation->galleries()->exists() ? Storage::url($recomendation->galleries->first()->url) : 'data:image/gif;base64,R0lGODlhAQABAIAAAAUEBAAAACwAAAAAAQABAAACAkQBADs=' }}"
                                    alt=""
                            ...
                            <a href="{{ route('frontend.details', $recomendation->slug) }}" class="stretched-link">
                                <!-- fake children -->
                            </a>
                        </div>
                    </div>
